  ...
  """
    ...
    for element in graphs_tuple_iterator:
        ...
        if _is_over_batch_size(element, valid_batch_size):
            ...
            batch_size = {
                k: v for k, v in zip(_NUMBER_FIELDS, valid_batch_size)
            }
            raise RuntimeError(
        ...
        if not accumulated_graphs:
            ...
            if ((num_accumulated_graphs + element_graphs > n_graph - 1)
                ...
                batched_graph = jraph.batch(accumulated_graphs)
                yield jraph.pad_with_graphs(batched_graph, n_node, n_edge,
                                            n_graph)
                accumulated_graphs = [element]
                num_accumulated_nodes = element_nodes
                num_accumulated_edges = element_edges
